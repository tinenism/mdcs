...
def save_downvote(request):
    ...
    return JsonResponse({'bool':True})
def undo_upvote(request):
    if request.method == 'POST':
        answer_id = request.POST.get('answerid')
        user = request.user
        
        try:
            upvote = UpVote.objects.get(answer_id=answer_id, user=user)
            upvote.delete()
            return JsonResponse({'bool': True})
        except UpVote.DoesNotExist:
            return JsonResponse({'bool': False})

def undo_downvote(request):
    if request.method == 'POST':
        answer_id = request.POST.get('answerid')
        user = request.user
        
        try:
            downvote = DownVote.objects.get(answer_id=answer_id, user=user)
            downvote.delete()
            return JsonResponse({'bool': True})
